...
class ContactFieldAdapter extends RecyclerView.Adapter<ContactFieldAdapter.ContactFieldViewHolder>{
                 ...
                 @NonNull List<PostalAddress> postalAddresses)
  {
    ...
    fields.addAll(Stream.of(phoneNumbers).map(phone -> new Field(context, phone)).toList());
  ...
  static class Field {
    ...
    Field(@NonNull Context context, @NonNull Phone phoneNumber) {
      ...
      switch (phoneNumber.getType()) {
        case HOME:
          label = context.getString(R.string.ContactShareEditFragment_type_home);
          break;
        case MOBILE:
          label = context.getString(R.string.ContactShareEditFragment_type_mobile);
          break;
        case WORK:
    ...
    Field(@NonNull Context context, @NonNull Email email) {
      ...
      this.iconResId  = R.drawable.ic_call_white_24dp; // TODO: Pick correct icon
      this.maxLines   = 1;
