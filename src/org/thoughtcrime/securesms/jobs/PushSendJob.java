...
//    String relay = TextSecureDirectory.getInstance(context).getRelay(address.toPhoneString());
  ...
  Optional<List<SharedContact>> getContactsFor(OutgoingMediaMessage message) {
    ...
    for (Contact contact : message.getOutgoingContacts()) {
      ...
      SharedContact.Name name = new SharedContact.Name.Builder().setDisplay(contact.getName().getGivenName())
